# -*- coding: utf-8 -*-
"""
Created on Wed Feb 12 18:05:08 2025
...
Addition 2024_07_22: added the a0 in parameters and in deriv and update

Correction 2024_10_09: correcting the consolidation pressure evolution

Adjustment 2024_12_24: pc expression in y related functions is simplified 
    by taking into account  alp_pc0(n) allowing to set pc as pc0 initially.
    The definition of alp_pc0(n) is done in the update function
        and depends on the density via lambda.

Adjustment 2025_02: making the model able to cope with anisotropic initial stress 
    by taking out of the model file the calculation of the initial specific volume 
    to make it an iterative trial and error routine able to cope with anisotropy.
    This means that pi and vi here are really the initial values for the test,
    that is then taken through the initiation routine (to what is now written as
    (pt,qt) and vt), and then through the simulated test.
    The initial alpha_pc achieves pc=5kPa.
...
def y(eps,sig,alp,chi):
    ...
    for i in range(NN):
        ...
        temp[i] += (NN*chi[NN+i,0]/pc[i])**r                # Consolidation part
        temp[i] -= 1
    return temp
def dyde(eps,sig,alp,chi):
    ...
    for i in range(NN):
        ...
        tempy[i,0] += (NN*chi[NN+i,0])**r * (-r) * dpc[i,0] * pc[i]**(-r-1) # Consolidation part
    return tempy
...
def dyda(eps,sig,alp,chi):
    ...
    for i in range(NN):
        for j in range(n_int):
            ...
            tempy[i,j] += (NN*chi[NN+i,0])**r * (-r) * dpc[i,j] * pc[i]**(-r-1) # Consolidation part
    return tempy
def dydc(eps,sig,alp,chi):
    ...
    for i in range(NN):
        for j in range(n_int):
            tempy[i,j] = 2*dCHI[i,j] *CHI[i] / ( ((i+1)*rN)**2 * MN_pq )    # Matsuoka-Nakai part
        tempy[i,NN+i,0] += (NN/pc[i])**r * r *chi[NN+i,0]**(r-1)            # Consolidation part
    return tempy